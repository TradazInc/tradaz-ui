...
const BASE_SUBSCRIPTIONS: Subscription[] = [
  {
    ...
    startedAt: "Oct 12, 2024",
  },
  {
    id: "SUB-8813",
    tenant: "Minimalist Hub",
    owner: "Sarah Connor",
    plan: "Enterprise",
    cycle: "Annually",
    amount: 1500000,
    status: "active",
    nextBilling: "Jan 01, 2027",
    startedAt: "Jan 01, 2024",
  },
  {
    id: "SUB-8814",
    tenant: "Lagos Streetwear",
    owner: "Tobi O.",
    plan: "Basic",
    cycle: "Monthly",
    amount: 15000,
    status: "past_due",
    nextBilling: "Overdue by 3 days",
    startedAt: "Mar 15, 2025",
  },
  {
    id: "SUB-8815",
    tenant: "Tech Gadgets Pro",
    owner: "John Doe",
    plan: "Pro",
    cycle: "Monthly",
    amount: 45000,
...
export const useAdminSubscriptions = () => {
  ...
  const [allSubscriptions, setAllSubscriptions] = useState<Subscription[]>(() =>
    generateExtendedSubscriptions(TOTAL_SUBSCRIPTIONS),
  );
  ...
  const processedSubscriptions = useMemo(() => {
    ...
    if (planFilter !== "all") {
      processed = processed.filter(
        (sub) => sub.plan.toLowerCase() === planFilter.toLowerCase(),
      );
    }
  ...
  const kpiStats = useMemo(() => {
    const activeSubs = processedSubscriptions.filter(
    ...
    const pastDueAmount = processedSubscriptions
      .filter((s) => s.status === "past_due")
      .reduce((acc, curr) => acc + curr.amount, 0);
    const pastDueCount = processedSubscriptions.filter(
      (s) => s.status === "past_due",
    ).length;

    const trialCount = processedSubscriptions.filter(
      (s) => s.status === "trialing",
    ).length;
    ...
    return [
      ...
      },
      {
        ...
        value: `₦${pastDueAmount.toLocaleString()}`,
        trend: `${pastDueCount} shops`,
        color: "red.400",
      },
      {
        label: "Active Trials",
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (
          ...
          visibleCount < processedSubscriptions.length &&
          !isLoadingMore
        ) {
          setIsLoadingMore(true);
          setTimeout(() => {
            setVisibleCount((prev) =>
              Math.min(prev + ITEMS_PER_PAGE, processedSubscriptions.length),
            );
  ...
  return {
    ...
    totalLimit: TOTAL_SUBSCRIPTIONS,
    visibleCount,
    isLoadingMore,
    loaderRef,
    kpiStats,
