...
export const useAdminTransactions = () => {
  ...
  const [allTransactions] = useState<Transaction[]>(() =>
    generateExtendedTransactions(TOTAL_TRANSACTIONS),
  );
  ...
  const kpiStats = useMemo(() => {
    ...
    const feesCollected = processedTransactions
      .filter((t) => t.status === "completed")
      .reduce((acc, curr) => acc + curr.platformFee, 0);

    const pendingPayoutTx = processedTransactions.filter(
      (t) => t.type === "payout" && t.status === "pending",
    );
    const pendingPayoutAmount = pendingPayoutTx.reduce(
    ...
    return [
      ...
      },
      {
        ...
        value: `₦${pendingPayoutAmount.toLocaleString()}`,
        trend: `${pendingPayoutTx.length} pending`,
      },
    ];
        ...
        if (
          ...
          setTimeout(() => {
            setVisibleCount((prev) =>
              Math.min(prev + ITEMS_PER_PAGE, processedTransactions.length),
            );
