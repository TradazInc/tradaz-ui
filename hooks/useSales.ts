...
export const useSales = () => {
  const [allSales, setAllSales] = useState<ExtendedSalesRecord[]>(() =>
    generateDummySales(TOTAL_SALES),
  );
  ...
  const [selectedSale, setSelectedSale] = useState<ExtendedSalesRecord | null>(
    null,
  );
    ...
    if (searchQuery) {
      ...
      processed = processed.filter(
        (s) =>
          ...
          s.transaction.toLowerCase().includes(q),
      );
    }
    processed.sort((a, b) => {
      let comparison = 0;
        ...
        if (
          ...
          setTimeout(() => {
            setVisibleCount((prev) =>
              Math.min(prev + ITEMS_PER_PAGE, processedSales.length),
            );
  ...
  return {
    ...
    handleSortOrder,
    visibleItems,
    processedSalesLength: processedSales.length,
    visibleCount,
    isLoadingMore,
    loaderRef,
    selectedSale,
