...
export const useDashboardData = () => {
  const [dbBusinesses, setDbBusinesses] =
    useState<Business[]>(DEFAULT_BUSINESSES);
  const [dbStores, setDbStores] =
    useState<Record<string, Store[]>>(DEFAULT_STORES);
  ...
  const activeBusiness =
    dbBusinesses.find((b) => b.id === activeBusinessId) || dbBusinesses[0];
  const availableStores = dbStores[activeBusiness?.id] || [];
  const activeStore =
    availableStores.find((s: Store) => s.id === activeStoreId) ||
    availableStores[0];
