...
const BASE_MOCK_STAFF: StaffMember[] = [
  ...
  },
  {
    id: "STF-002",
    name: "David O.",
    email: "david.o@urbankicks.com",
    role: "sales_rep",
    status: "active",
    salesGenerated: "₦1,250,000",
    lastActive: "2 hours ago",
  },
  {
    id: "STF-003",
    name: "Amina Yusuf",
    email: "amina@urbankicks.com",
    role: "sales_rep",
    status: "active",
    salesGenerated: "₦850,000",
    lastActive: "5 mins ago",
  },
  {
    id: "STF-004",
    name: "Chinedu Eze",
    email: "chinedu@urbankicks.com",
...
export const useStaff = () => {
  const [allStaff] = useState<StaffMember[]>(() =>
    generateExtendedStaff(TOTAL_STAFF),
  );
  ...
  const activeSalesRepsCount = processedStaff.filter(
    (s) => s.role === "sales_rep" && s.status === "active",
  ).length;
  const pendingInvitesCount = processedStaff.filter(
    (s) => s.status === "invited",
  ).length;
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (
          ...
          visibleCount < processedStaff.length &&
          !isLoadingMore
        ) {
          setIsLoadingMore(true);
          setTimeout(() => {
            setVisibleCount((prev) =>
              Math.min(prev + ITEMS_PER_PAGE, processedStaff.length),
            );
  ...
  return {
    ...
    totalLimit: TOTAL_STAFF,
    visibleCount,
    isLoadingMore,
    loaderRef,
    activeSalesRepsCount,
    pendingInvitesCount,
  };
};
