...
export const useExpenses = () => {
  ...
  const [allExpenses] = useState<Expense[]>(() =>
    generateDummyExpenses(TOTAL_EXPENSES_LIMIT, 0),
  );
  ...
  const totalExpenses = processedExpenses.reduce(
    (acc, curr) => acc + curr.amount,
    0,
  );
  const pendingExpenses = processedExpenses
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (
          ...
          visibleCount < processedExpenses.length &&
          !isLoadingMore
        ) {
          setIsLoadingMore(true);
          setTimeout(() => {
            setVisibleCount((prev) =>
              Math.min(prev + ITEMS_PER_PAGE, processedExpenses.length),
            );
  ...
  return {
    ...
    handleSortOrder,
    visibleItems,
    processedExpensesLength: processedExpenses.length,
    totalLimit: TOTAL_EXPENSES_LIMIT,
    visibleCount,
    isLoadingMore,
    loaderRef,
    totalExpenses,
