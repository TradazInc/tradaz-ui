...
export const useStaffSalaries = () => {
  ...
  const [allSalaries] = useState<StaffSalary[]>(() =>
    generateDummyStaffSalaries(TOTAL_STAFF, 0),
  );
  ...
  const totalPayroll = processedSalaries.reduce(
    (acc, curr) => acc + curr.salary,
    0,
  );
  const totalCleared = processedSalaries
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (
          ...
          visibleCount < processedSalaries.length &&
          !isLoadingMore
        ) {
          setIsLoadingMore(true);
          setTimeout(() => {
            setVisibleCount((prev) =>
              Math.min(prev + ITEMS_PER_PAGE, processedSalaries.length),
            );
  ...
  return {
    ...
    handleSortOrder,
    visibleItems,
    processedSalariesLength: processedSalaries.length,
