...
export const useRevenue = () => {
  //  Data State
  const [allTransactions] = useState<RevenueTransaction[]>(() =>
    generateDummyRevenue(TOTAL_LIMIT, 0),
  );
  ...
  const processedTransactions = useMemo(() => {
    ...
    if (channelFilter !== "all") {
      processed = processed.filter(
        (t) => t.channel.toLowerCase() === channelFilter.toLowerCase(),
      );
    }
  ...
  const onlineSales = processedTransactions
    .filter((t) => t.channel === "Online" && t.status !== "Refunded")
    .reduce((acc, curr) => acc + curr.amount, 0);
  const posSales = processedTransactions
    .filter((t) => t.channel === "POS" && t.status !== "Refunded")
    .reduce((acc, curr) => acc + curr.amount, 0);
