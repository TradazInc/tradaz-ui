...
export default function StorefrontHomePage() {
  ...
  const [quickAddProduct, setQuickAddProduct] = useState<ProductType | null>(
    null,
  );
  ...
  useEffect(() => {
    try {
      const storedBanners = JSON.parse(
        localStorage.getItem("tradaz_banners") || "[]",
      );
  ...
  const topBarBanners = adminBanners.filter(
    (b) => b.status === "Active" && b.position === "Top Announcement Bar",
  );
  const customHeroBanners = adminBanners.filter(
    (b) => b.status === "Active" && b.position === "Hero Slider",
  );
  ...
  const confirmAddToCart = () => {
    ...
    try {
      const existingCart: CartItem[] = JSON.parse(
        localStorage.getItem("tradaz_cart") || "[]",
      );

      const existingItemIndex = existingCart.findIndex(
        (item: CartItem) =>
          item.id === quickAddProduct.id &&
      ...
      setToastMessage(
        `${quantity}x ${quickAddProduct.name} (${selectedVariation}) added to cart!`,
      );
      setTimeout(() => setToastMessage(null), 3000);
    ...
    <Box w="full" position="relative">
      ...
      {toastMessage && (
        <Box
          ...
          bg="#1A1C23"
          border="1px solid"
          borderColor={toastType === "success" ? brandColor : "red.400"}
          shadow="2xl"
          px={6}
          py={4}
          rounded="xl"
        ...
        >
          <Flex align="center" gap={3}>
            ...
            >
              <Icon
                as={toastType === "success" ? LuCheck : LuX}
                color={toastType === "success" ? brandColor : "red.400"}
                boxSize="14px"
                strokeWidth="3"
              />
            </Flex>
            ...
            <Flex justify="space-between" align="start" mb={6}>
              <Flex gap={4}>
                ...
                <Box>
                  ...
                  <Text color={brandColor} fontWeight="black" fontSize="lg">
                    ₦{quickAddProduct.price.toLocaleString()}
                  </Text>
                </Box>
              </Flex>
              <IconButton
              ...
              <Flex wrap="wrap" gap={2}>
                {MOCK_VARIATIONS.map((size) => (
                  <Button
                    ...
                    border="1px solid"
                    borderColor={
                      selectedVariation === size ? "white" : "#333333"
                    }
                    onClick={() => setSelectedVariation(size)}
                    _hover={{
                      bg: selectedVariation === size ? "white" : "#1A1A1A",
                    }}
                  >
                    {size}
                  </Button>
        ...
        >
          ...
          {carouselItems.map((banner, index) => (
            ...
            >
              ...
              {banner.image ? (
                <>
                  <Box
                    position="absolute"
                    inset={0}
                    bgGradient="linear(to-t, #0A0A0B 0%, transparent 70%)"
                    zIndex={1}
                  />
                  <Image
                    src={banner.image}
              ...
              >
                <Badge
                  bg={banner.isCustom ? "white" : brandColor}
                  color={banner.isCustom ? "black" : "white"}
                  px={3}
                  py={1}
                  rounded="full"
                  mb={4}
                ...
                <Text
                  ...
                  fontWeight="black"
                  color={banner.isCustom ? banner.textColor : "white"}
                  lineHeight="1.1"
                  mb={4}
                >
                  {banner.title}
                ...
                <Text
                  ...
                  color={banner.isCustom ? banner.textColor : "gray.300"}
                  opacity={banner.isCustom ? 0.9 : 1}
                  mb={8}
                  maxW="md"
                >
                  {banner.subtitle}
          ...
          >
            {carouselItems.map((_, idx) => (
              <Box
                key={idx}
                boxSize={currentSlide === idx ? "10px" : "8px"}
                bg={currentSlide === idx ? "white" : "whiteAlpha.400"}
                rounded="full"
                cursor="pointer"
                transition="all 0.3s"
