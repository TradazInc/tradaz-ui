...
const INITIAL_CATEGORIES = [
    { id: "1", name: "Foot Wears", code: "CAT-FW", slug: "foot-wears", description: "All types of shoes, sneakers, and boots.", created: "May 10, 2026" },
    { id: "2", name: "Sport Wears", code: "CAT-SW", slug: "sport-wears", description: "Athletic clothing, gym gear, and activewear.", created: "May 08, 2026" },
    { id: "3", name: "Accessories", code: "CAT-ACC", slug: "accessories", description: "Jewelry, watches, belts, and hats.", created: "Apr 22, 2026" },
    { id: "4", name: "Bags", code: "CAT-BG", slug: "bags", description: "Backpacks, handbags, and travel luggage.", created: "Apr 15, 2026" },
    { id: "5", name: "Electronics", code: "CAT-EL", slug: "electronics", description: "Gadgets, devices, and tech accessories.", created: "Mar 30, 2026" }
];
...
export default function CategoriesPage() {
    ...
    const handleAddCategory = () => {
        ...
        const generatedSlug = newCategory.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
    ...
    const handleUpdateCategory = () => {
        if (!editingCategory || !editingCategory.name.trim() || !editingCategory.code.trim()) return;

        const updatedSlug = editingCategory.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
    ...
    return (
        <Box p={{ base: 4, lg: 8 }} maxW="1600px" mx="auto" animation="fade-in 0.3s ease" bg="#000000" minH="100vh">
                ...
                <Box position="fixed" inset={0} zIndex={9999} bg="blackAlpha.800" backdropFilter="blur(4px)" display="flex" alignItems="center" justifyContent="center" p={4}>
                    <Box bg="#0A0A0A" border="1px solid #333333" rounded="none" w="full" maxW="500px" p={6} shadow="2xl">
                        ...
                        <VStack gap={5} align="stretch">
                            <Box>
                                <Text color="#888888" fontSize="xs" fontWeight="bold" mb={2} textTransform="uppercase" letterSpacing="wider">Category Name</Text>
                                <Input 
                            ...
                            <Box>
                                <Text color="#888888" fontSize="xs" fontWeight="bold" mb={2} textTransform="uppercase" letterSpacing="wider">Category Code</Text>
                                <Input 
                        ...
                        <Flex justify="flex-end" gap={3} mt={8}>
                            ...
                            <Button onClick={handleAddCategory} bg="white" color="black" rounded="none" _hover={{ bg: "#e0e0e0" }}>Save Category</Button>
                        </Flex>
                    </Box>
                </Box>
            )}
            ...
            {isEditModalOpen && editingCategory && (
                <Box position="fixed" inset={0} zIndex={9999} bg="blackAlpha.800" backdropFilter="blur(4px)" display="flex" alignItems="center" justifyContent="center" p={4}>
                    <Box bg="#0A0A0A" border="1px solid #333333" rounded="none" w="full" maxW="500px" p={6} shadow="2xl">
                        <Text fontSize="xl" fontWeight="black" color="white" mb={6} letterSpacing="tight">Edit Category</Text>
                        ...
                        <Flex justify="flex-end" gap={3} mt={8}>
                            ...
                            <Button onClick={handleUpdateCategory} bg="white" color="black" rounded="none" _hover={{ bg: "#e0e0e0" }}>Update Category</Button>
                        </Flex>
                    </Box>
                </Box>
            )}
            ...
            <Flex justify="space-between" align={{ base: "flex-start", md: "flex-end" }} direction={{ base: "column", md: "row" }} gap={4} mb={8}>
                <Box>
                    <Flex align="center" gap={3} mb={1}>
                        <Icon as={LuList} color={brandColor} boxSize="24px" strokeWidth="2.5" />
                        <Text fontSize={{ base: "2xl", md: "3xl" }} fontWeight="black" color="white" letterSpacing="tight">
                            Categories
                        </Text>
                    </Flex>
                    <Text color="#888888" fontSize="sm">Manage product categories, codes, and classifications.</Text>
                </Box>
                ...
                <Box bg="#0A0A0A" p={6} rounded="none" border="1px solid" borderColor="#1A1A1A">
                    <Flex justify="space-between" align="start" mb={4}>
                        <Flex boxSize="40px" bg="#111111" border="1px solid #333333" rounded="none" align="center" justify="center">
                            <Icon as={LuPackage} color={brandColor} boxSize="20px" strokeWidth="2.5" />
                        </Flex>
                    </Flex>
                    <Text color="#888888" fontSize="10px" fontWeight="bold" textTransform="uppercase" letterSpacing="wider" mb={1}>Total Available Categories</Text>
                    <Flex align="baseline" gap={3}>
                        <Text color="white" fontSize="3xl" fontWeight="black" letterSpacing="tight">{categories.length}</Text>
                    </Flex>
                </Box>
            </SimpleGrid>
            ...
            <Box bg="#0A0A0A" border="1px solid #1A1A1A" overflowX="auto">
                <Box minW="1000px">
                    {/* Table Header */}
                    <Grid templateColumns="1.5fr 1fr 1fr 2.5fr 1fr 0.5fr" gap={4} px={6} py={4} bg="#111111" borderBottom="1px solid #333333">
                        ...
                        <Text color="#888888" fontSize="10px" fontWeight="bold" textTransform="uppercase" letterSpacing="wider">Slug</Text>
                    ...
                    <VStack align="stretch" gap={0}>
                        {filteredCategories.length > 0 ? (
                            filteredCategories.map((category) => (
                                <Grid 
                                    key={category.id} templateColumns="1.5fr 1fr 1fr 2.5fr 1fr 0.5fr" gap={4} px={6} py={4} 
                                    borderBottom="1px solid #1A1A1A" alignItems="center" 
                                    _hover={{ bg: "#111111" }} transition="background 0.2s"
                                >
                                    <Text color="white" fontSize="sm" fontWeight="bold">{category.name}</Text>
                                    ...
                                    <Text color="#888888" fontSize="12px" fontFamily="monospace">/{category.slug}</Text>
                                    
                                    <Text color="#888888" fontSize="sm"title={category.description}>
                                        {category.description}
                                    </Text>
                                    
                                    <Text color="white" fontSize="xs" fontWeight="medium">{category.created}</Text>
                            ...
                            <Flex justify="center" align="center" py={12} direction="column" gap={3}>
                                ...
                                <Text color="#888888" fontSize="sm">No categories found matching your criteria.</Text>
                            </Flex>
                        )}
                    </VStack>
                </Box>
