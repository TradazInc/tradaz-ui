  ...
  if (customerQuery !== prevQuery.customer || orderQuery !== prevQuery.order) {
    ...
    if (customerQuery) {
      const foundChat = chats.find(
        (c) => c.customerName.toLowerCase() === customerQuery.toLowerCase(),
      );
      ...
      const foundChat = chats.find(
        (c) =>
          c.id === orderQuery ||
          (c as CustomerChat & { orderId?: string }).orderId === orderQuery,
      );
      if (foundChat) foundChatId = foundChat.id;
    }
    ...
    if (foundChatId && activeChatId !== foundChatId) {
      ...
      setChats((prev) =>
        prev.map((c) =>
          c.id === foundChatId && c.status === "Unread"
            ? { ...c, status: "Pending" }
            : c,
        ),
      );
            ...
            <Flex w="full" gap={2}>
              <select
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                  setStatusFilter(
                    e.target.value as "All" | "Unread" | "Pending" | "Replied",
                  )
                }
              >
                ...
                <option value="Unread" style={{ background: "#0A0A0A" }}>
                  Unread First
                </option>
                <option value="Pending" style={{ background: "#0A0A0A" }}>
                  Pending
                </option>
                <option value="Replied" style={{ background: "#0A0A0A" }}>
                  Replied
                </option>
              </select>
            </Flex>
        ...
        {!activeChat ? (
          ...
          <>
            ...
            >
              ...
              <Badge
                bg={
                  activeChat.status === "Replied"
                    ? "rgba(92, 172, 125, 0.1)"
                    : activeChat.status === "Unread"
                      ? "rgba(237, 137, 54, 0.1)"
                      : "#111111"
                }
                color={
                    ...
                    >
                      <Box maxW="70%">
                        <Box
                          ...
                          border="1px solid"
                          borderColor={isAdmin ? "white" : "#333333"}
