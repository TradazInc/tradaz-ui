    ...
    >
      ...
      >
        <Flex align="center" gap={3} mb={4}>
          <Icon
            as={LuTriangle}
            color={isDelete ? "red.400" : "orange.400"}
            boxSize="24px"
          />
          <Text color="white" fontWeight="bold" fontSize="lg">
            {isDelete ? "Delete" : "Suspend"} Customer?
          </Text>
        </Flex>
        <Text color="gray.400" mb={6} fontSize="sm">
          Are you sure you want to {action} <b>{customer.name}</b>?{" "}
          {isDelete
            ? "This action cannot be undone."
            : "They will not be able to log in or make purchases."}
        </Text>
