...
export const RefundManager = () => {
  const [refunds, setRefunds] = useState<RefundRequest[]>(
    generateDummyRefunds(),
  );
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState<
    "All" | "Pending" | "Processing" | "Refunded" | "Rejected"
  >("All");

  // --- ACTIONS ---
  const updateStatus = (
    id: string,
    newStatus: "Processing" | "Refunded" | "Rejected",
  ) => {
    setRefunds((prev) =>
  ...
  const issueStoreCredit = (
    ...
    alert(
      `Successfully generated a ₦${amount.toLocaleString()} Gift Voucher for ${customerName}! They will receive an email shortly.`,
    );
        ...
        >
          <Box>
            <Text
              ...
              letterSpacing="tight"
            >
              <Icon as={LuUndo2} color="#5cac7d" strokeWidth="2.5" /> Customer
              Refunds
            </Text>
            <Text color="#888888" fontSize="sm">
              Process money back requests or issue store credit to retain value.
            </Text>
          </Box>
        </Flex>
      </Box>
      ...
      <SimpleGrid columns={{ base: 1, md: 3 }} gap={4} mb={8}>
        ...
        >
          ...
          <Text color="#888888" fontSize="xs" mt={1}>
            Across {pendingRefunds.length} request(s)
          </Text>
        </Box>
        <Box
          bg="#0A0A0A"
        ...
        <Flex direction="column" justify="center" gap={3}>
          ...
          >
            ...
            <option value="Processing" style={{ background: "#0A0A0A" }}>
              Processing Return
            </option>
            <option value="Refunded" style={{ background: "#0A0A0A" }}>
              Refunded / Credited
            </option>
            <option value="Rejected" style={{ background: "#0A0A0A" }}>
              Rejected
            </option>
          </select>
        </Flex>
          ...
          filteredRefunds.map((ref) => {
            ...
            return (
              <Box
                ...
                p={{ base: 4, md: 6 }}
                transition="all 0.2s"
                opacity={isRejected || isRefunded ? 0.6 : 1}
                _hover={{ bg: "#111111" }}
              >
                <Flex
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <Text
                        color={isRefunded ? "white" : "red.400"}
                        fontSize="lg"
                        fontWeight="black"
                        letterSpacing="tight"
                      >
                  ...
                  >
                    ...
                    >
                      <Icon
                        ...
                        strokeWidth="2.5"
                      />
                      <Text color="#888888" fontSize="xs" fontStyle="italic">
                        &quot;{ref.reason}&quot;
                      </Text>
                    </Flex>
                    ...
                    >
                      ...
                      <Button
                        size="sm"
                        h="36px"
                        onClick={() => openChatHistory(ref.customerName)}
                        variant="outline"
                        borderColor="#333333"
                        color="white"
                        rounded="none"
                        _hover={{ bg: "#111111" }}
                      >
                        <Icon as={LuMessageCircle} mr={2} strokeWidth="2.5" />{" "}
                        View Chat
                      </Button>
                        ...
                        {isPending && (
                          <>
                            ...
                            <Button
                              size="sm"
                              h="36px"
                              onClick={() => updateStatus(ref.id, "Processing")}
                              bg="#111111"
                              color="blue.400"
                              border="1px solid #333333"
                              rounded="none"
                        ...
                        {isProcessing && (
                          <>
                            <Button
                              ...
                              color="#5cac7d"
                              rounded="none"
                              _hover={{ bg: "rgba(92, 172, 125, 0.1)" }}
                            >
                              <Icon as={LuGift} mr={2} strokeWidth="2.5" />{" "}
                              Issue Credit
                            </Button>
                            <Button
                              size="sm"
                              h="36px"
                              onClick={() => updateStatus(ref.id, "Refunded")}
                              bg="#111111"
                              border="1px solid #333333"
                              color="red.400"
                              rounded="none"
                              _hover={{ bg: "red.900", color: "white" }}
                            >
                              <Icon as={LuBanknote} mr={2} strokeWidth="2.5" />{" "}
                              Refund Cash
                            </Button>
                          </>
                        )}
