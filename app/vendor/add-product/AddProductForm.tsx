...
export const AddProductForm = () => {
  ...
  const steps = [
    { name: "Media", icon: LuImage, isComplete: mediaFiles.length > 0 },
    {
      name: "Basic Info",
      icon: LuFolder,
      isComplete: basicInfo.name.length > 0 && basicInfo.brand.length > 0,
    },
    ...
    <Box w="full" pb={10} position="relative" bg="#000000">
      ...
      >
        <Box
          ...
          p={5}
          border="1px solid"
          borderColor={incompleteSteps === 0 ? "white" : "#1A1A1A"}
        >
          <Flex justify="space-between" align="center" wrap="wrap" gap={4}>
            {steps.map((step, idx) => (
              <Flex key={idx} align="center" gap={2}>
                ...
                <Text
                  fontSize="sm"
                  color={step.isComplete ? "white" : "gray.500"}
                  fontWeight="medium"
                  transition="color 0.3s ease"
                >
                  {step.name}
          ...
          {incompleteSteps > 0 ? (
            ...
            >
              ...
              <Text color="white" fontSize="sm" fontWeight="bold">
                All set! You can now submit your product for review.
              </Text>
            </Flex>
          )}
        </Box>
      ...
      <Box bg="#0A0A0A" rounded="none" p={6} mb={8} border="1px solid #1A1A1A">
        ...
        <Box
          border="1px dashed"
          borderColor={mediaFiles.length > 0 ? "white" : "#333333"}
          rounded="none"
          bg="#111111"
          _hover={{ borderColor: "white", bg: "#1A1A1A" }}
          transition="all 0.2s"
        >
          {mediaFiles.length === 0 ? (
            <Flex
            ...
            <Box p={6}>
              <SimpleGrid columns={{ base: 2, md: 4, lg: 6 }} gap={4} mb={6}>
                ...
                <Flex
                  ...
                  aspectRatio="1"
                  cursor="pointer"
                  _hover={{ bg: "#111111", borderColor: "white" }}
                  onClick={() => fileInputRef.current?.click()}
                >
        ...
        >
          ...
          <Box>
            ...
            <Text color="gray.400" fontSize="xs" mt={1}>
              Once you click &quot;Submit for Review&quot;, this product will be
              sent to the merchant for verification. Your product will only go
              live in the store after it has been approved.
            </Text>
          </Box>
        </Flex>
