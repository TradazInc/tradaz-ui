...
export default function StorefrontHomePage() {
    ...
    return (
        <Box p={{ base: 4, lg: 8 }} w="full" mx="auto">
            ...
            <Box 
                position="relative" w="full" h={{ base: "350px", md: "450px", lg: "550px" }} 
                rounded="3xl" overflow="hidden" mb={12} shadow="2xl"
            >
                {STORE_BANNERS.map((banner, index) => (
                    ...
                    >
                        ...
                        <Flex 
                            ...
                            p={{ base: 6, lg: 12 }} direction="column" align="flex-start" w={{ base: "90%", lg: "60%" }}
                        >
                            <Badge bg={brandColor} color="white" px={3} py={1} rounded="full" mb={4} fontSize="xs" fontWeight="bold" letterSpacing="widest" textTransform="uppercase">
                                Limited Time!
                            </Badge>
                            <Text fontSize={{ base: "4xl", lg: "6xl" }} fontWeight="black" color="white" lineHeight="1.1" mb={4}>
                                {banner.title}
                            </Text>
                            <Text fontSize={{ base: "lg", lg: "xl" }} color="gray.300" mb={8} maxW="md">
                                {banner.subtitle}
                            </Text>
                            
                            <Button 
                                display="flex" alignItems="center" gap={3}
                                bg={brandColor} color="white" px={8} py={6} rounded="full" 
                ...
                <Flex position="absolute" bottom={6} left="50%" transform="translateX(-50%)" gap={3} zIndex={10}>
                    {STORE_BANNERS.map((_, idx) => (
                        <Box 
                            key={idx} boxSize={currentSlide === idx ? "10px" : "8px"} 
                            bg={currentSlide === idx ? brandColor : "whiteAlpha.400"} 
                            rounded="full" cursor="pointer" transition="all 0.3s"
            ...
            >
                {featuredProducts.map((product) => (
                    <Box key={`feat-${product.id}`} role="group" position="relative" rounded="3xl" overflow="hidden" cursor="pointer" h={{ base: "400px", md: "500px", lg: "65vh" }}>
                        <Image 
                        ...
                        <Flex position="absolute" bottom={0} left={0} right={0} p={6} direction="column" justify="flex-end">
                            <Text fontSize="xs" color={brandColor} textTransform="uppercase" fontWeight="bold" letterSpacing="widest" mb={1}>
                                {product.category}
                            </Text>
                            <Text fontSize="2xl" fontWeight="black" color="white" lineHeight="1.1" mb={2} lineClamp={2}>
                                {product.name}
                            </Text>
                            <Flex justify="space-between" align="center" mt={2}>
                                ...
                                <Button size="sm" rounded="full" bg={brandColor} color="white" _hover={{ filter: "brightness(1.1)" }} display="flex" gap={2}>
            ...
            <Box borderTop="1px solid" borderColor="whiteAlpha.100" pt={10}>
                ...
                <Grid templateColumns={{ base: "repeat(2, 1fr)", md: "repeat(3, 1fr)", lg: "repeat(4, 1fr)", xl: "repeat(4, 1fr)" }} gap={{ base: 4, lg: 6 }}>
                    {feedProducts.map((product, index) => (
                        <Box 
                            ...
                            transition="all 0.2s"
                            _hover={{ borderColor: brandColor, transform: "translateY(-4px)", shadow: "xl" }}
                        >
                            {/* Image Container */}
                            <Box position="relative" rounded="xl" overflow="hidden" bg="whiteAlpha.50" mb={4} aspectRatio={1}>
                                {/* Top Left Badge */}
                                <Badge position="absolute" top={2} left={2} bg={brandColor} color="white" rounded="sm" px={2} py={0.5} fontSize="9px" fontWeight="bold" zIndex={10}>
                                    {product.isNew ? "NEW" : "TRENDING"}
                                </Badge>
                                
                                {/* Top Right Heart */}
                                <IconButton 
                                    position="absolute" top={2} right={2} zIndex={10}
                                    aria-label="Wishlist" size="sm" rounded="full" bg="blackAlpha.500" color="white" 
                                    backdropFilter="blur(4px)" _hover={{ bg: "red.500", color: "white" }}
                                >
                            ...
                            <Box px={1}>
                                ...
                                <Text fontSize="lg" fontWeight="black" color={brandColor} mb={4}>
                                    ₦{product.price.toLocaleString()}
                                </Text>
