...
import { Box, Flex, Text, SimpleGrid, Icon, Input, Button, Image, Spinner } from "@chakra-ui/react";
import { LuSearch, LuStar, LuPrinter, LuPackagePlus, LuPencil, LuTrash2 } from "react-icons/lu";
...
const nativeSelectStyle: React.CSSProperties = {
    width: "100%", backgroundColor: "#121214", color: "white", height: "40px", borderRadius: "8px", padding: "0 12px", border: "1px solid rgba(255, 255, 255, 0.2)", outline: "none", cursor: "pointer"
};
...
export const InventoryGridView = ({
    ...
    handleSearch, handleFilter, handleSort, onSelectProduct,
    ...
    return (
        <Box w="full" maxW="100%" display="flex" flexDirection="column" position="relative">
            ...
            <Box mb={4} pt={2}>
                <Text color="#5cac7d" fontWeight="bold" fontSize="3xl" mb={1}>Inventory</Text>
            ...
            <Box position="sticky" top={{ base: "70px", md: "85px" }} zIndex={20} bg="rgba(18, 18, 20, 0.85)" backdropFilter="blur(12px)" py={3} mb={6} borderBottom="1px solid" borderColor="whiteAlpha.100" w="full">
                <Flex direction={{ base: "column", md: "row" }} justify="space-between" align="center" gap={4} w="full">
                    <Flex w="full" maxW={{ base: "full", md: "400px" }} align="center" {...controlStyles} _focusWithin={{ borderColor: "#5cac7d" }}>
                        <Icon as={LuSearch} color="gray.400" />
                        <Input placeholder="Search by name or SKU..." border="none" _focus={{ outline: "none", boxShadow: "none" }} color="white" h="full" value={searchQuery} onChange={handleSearch} />
                    </Flex>
                    
                    <Flex gap={3} w={{ base: "full", md: "auto" }}>
                        <Box flex={1} w={{ base: "full", md: "180px" }}>
                            <select value={filterCategory} onChange={handleFilter} style={nativeSelectStyle}>
                                <option value="all" style={{ background: "#1A1C23" }}>All Products</option>
                                <option value="featured" style={{ background: "#1A1C23" }}>Featured Only</option>
                                <option value="low-stock" style={{ background: "#1A1C23" }}>Low Stock</option>
                                <option value="out-of-stock" style={{ background: "#1A1C23" }}>Out of Stock</option>
                            </select>
                        </Box>
                        <Box flex={1} w={{ base: "full", md: "180px" }}>
                            <select value={sortBy} onChange={handleSort} style={nativeSelectStyle}>
                                <option value="default" style={{ background: "#1A1C23" }}>Sort: Default</option>
                                <option value="price-asc" style={{ background: "#1A1C23" }}>Price: Low to High</option>
                                <option value="price-desc" style={{ background: "#1A1C23" }}>Price: High to Low</option>
                                <option value="stock-asc" style={{ background: "#1A1C23" }}>Stock: Low to High</option>
                                <option value="stock-desc" style={{ background: "#1A1C23" }}>Stock: High to Low</option>
                            </select>
                        </Box>
                    </Flex>
                </Flex>
            ...
            {visibleItems.length === 0 ? (
                <Flex justify="center" align="center" py={20} direction="column">
                    <Text color="gray.400" fontSize="lg" fontWeight="bold">No products found.</Text>
                ...
                <SimpleGrid columns={{ base: 1, sm: 2, lg: 3 }} gap={6} mb={8} w="full">
                    ...
                    {visibleItems.map((product) => (
                        <Box key={product.id} bg="#1A1C23" rounded="2xl" border="1px solid" borderColor="whiteAlpha.100" overflow="hidden" _hover={{ borderColor: "whiteAlpha.300", shadow: "2xl" }} transition="all 0.2s">
                            ...
                            <Box position="relative" h="280px" w="full" bg="#121214" cursor="pointer" onClick={() => onSelectProduct(product)} role="group">
                                ...
                                {product.isFeatured && (
                                    <Box position="absolute" top={3} right={3} bg="blackAlpha.800" color="white" fontSize="xs" fontWeight="bold" px={3} py={1} rounded="md" backdropFilter="blur(4px)">Featured</Box>
                                )}
                            </Box>

                            <Box p={5}>
                                <Flex justify="space-between" align="start" mb={4}>
                                    <Text color="white" fontWeight="bold" fontSize="lg" lineClamp={1} flex={1} pr={2}>{product.name}</Text>
