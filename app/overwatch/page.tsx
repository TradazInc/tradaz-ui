...
import { PlatformUser } from "./users/types";
...
export default function UsersPage() {
  ...
  const handleUpdateUser = async (id: string, updates: Partial<PlatformUser>, banReason?: string) => {
    try {
      if (updates.role && updates.role !== managingUser?.role) {
        const backendRole = updates.role.toLowerCase() === "admin" ? "admin" : "user";
        await actions.setRole.mutateAsync({ userId: id, role: backendRole as "admin" | "user" });
      }
      ...
      toaster.create({ title: "User updated", description: "Modifications saved.", type: "success" });
    } catch {
      toaster.create({ title: "Update failed", description: "Please try again.", type: "error" });
    }
  };
  ...
  const handleDeleteUser = async (id: string) => {
    ...
    try {
      ...
      toaster.create({ title: "User deleted", description: "Account removed permanently.", type: "success" });
      setManagingUser(null);
    } catch {
      toaster.create({ title: "Delete failed", description: "Could not delete user.", type: "error" });
    }
  };
  ...
  const handleSetPassword = async (id: string) => {
    ...
    try {
      await actions.setPassword.mutateAsync({ userId: id, newPassword: newPassword });
      toaster.create({ title: "Password updated", description: "User must use the new password.", type: "success" });
    } catch {
      toaster.create({ title: "Password reset failed", description: "Check the input.", type: "error" });
    }
  };
  ...
  const handleRevokeSessions = async (id: string) => {
    ...
    try {
      ...
      toaster.create({ title: "Sessions revoked", description: "User logged out everywhere.", type: "success" });
    } catch {
      toaster.create({ title: "Revoke failed", description: "Could not revoke sessions.", type: "error" });
    }
  };
  ...
  const handleImpersonate = async (id: string) => {
    ...
    try {
      const response = await actions.impersonateUser.mutateAsync({ userId: id });
      const token = (response as unknown as { session?: { token: string } })?.session?.token || "Active";
      toaster.create({ title: "Impersonation active", description: `Session token: ${token}`, type: "info" });
    } catch {
      toaster.create({ title: "Impersonation failed", description: "Insufficient permissions?", type: "error" });
    }
  };
  ...
  const queryData = data as unknown as {
    data?: { users?: RawAuthUser[] };
    users?: RawAuthUser[];
  };

  const rawUsers: RawAuthUser[] = queryData?.data?.users || queryData?.users || [];
  ...
  const mappedUsers: PlatformUser[] = rawUsers.map((user) => ({
    ...
    lastActive: user.updatedAt ? new Date(user.updatedAt).toLocaleDateString() : "Unknown",
  }));
  ...
  return (
    <Box p={{ base: 4, lg: 8 }} maxW="1400px" mx="auto" animation="fade-in 0.3s ease" bg="#000000" minH="100vh">
      <Flex justify="space-between" align={{ base: "flex-start", md: "flex-end" }} direction={{ base: "column", md: "row" }} gap={4} mb={8}>
        ...
        <Flex gap={3}>
          <Button onClick={handleExport} loading={isExporting} loadingText="Exporting..." display={{ base: "none", sm: "flex" }} bg="#111111" border="1px solid #333333" color="white" rounded="none" _hover={{ bg: "#1A1A1A" }} gap={2} h="44px" px={6} fontWeight="bold">
      ...
      <Box bg="#0A0A0A" border="1px solid #1A1A1A" p={5} mb={8}>
        <Flex direction={{ base: "column", md: "row" }} gap={4} w="full" justify="space-between">
          <Flex flex={1} maxW={{ md: "400px" }} align="center" {...controlStyles}>
            ...
            <Input placeholder="Search by name, email, or ID..." border="none" color="white" h="full" px={0} _focus={{ boxShadow: "none", outline: "none" }} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </Flex>
                ...
                {filteredUsers.length === 0 ? (
                  ...
                  <VStack align="stretch" gap={0}>
                    {filteredUsers.map((user: PlatformUser) => {
                      ...
                      return (
                        <Grid key={user.id} templateColumns="2.5fr 1fr 1fr 1fr 100px" gap={4} px={6} py={5} borderBottom="1px solid #1A1A1A" alignItems="center" _hover={{ bg: "#111111" }} transition="background 0.2s">
                          ...
                          <Flex justify="flex-end" gap={2}>
                            <IconButton onClick={() => setViewingUser(user)} aria-label="View Data" size="sm" h="36px" w="36px" bg="#111111" border="1px solid #333333" color="white" rounded="none" _hover={{ bg: "#1A1A1A", color: "white" }}>
                              <Icon as={LuEye} strokeWidth="2.5" />
                            </IconButton>
                            <IconButton onClick={() => setManagingUser(user)} aria-label="Manage Account" size="sm" h="36px" w="36px" variant="outline" borderColor="#333333" color="#888888" rounded="none" _hover={{ bg: "#111111", color: "white" }}>
